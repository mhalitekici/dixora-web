...
from app.models import ApprovalRequest, DiningTable, Order, OrderItem, TableSession
from app.models.enums import OrderSource, OrderStatus, TableState
    ...
    approval = (
        await db.execute(
            select(ApprovalRequest).where(
                ...
                ApprovalRequest.tenant_id == require_tenant(identity),
            )
        )
    ).scalar_one_or_none()
    if approval is None:
